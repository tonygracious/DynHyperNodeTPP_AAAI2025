...
class Memory(nn.Module):
  ...
  def __init__(self, n_nodes, memory_dimension, input_dimension, message_dimension=None,
    ...
    self.memory = nn.Parameter(torch.zeros(size= (self.n_nodes, self.memory_dimension)), requires_grad=False)
  ...
  def __init_memory__(self, embed):
    """
    Initializes the memory to all zeros. It should be called at the start of each epoch.
    """
    # Treat memory as parameter so that it is saved and loaded together with the model
    #embed = F.normalize(torch.from_numpy(embed).to(self.device))
    embed = torch.from_numpy(embed).to(self.device)
    embed[0] = 0
    self.memory = nn.Parameter(embed, requires_grad=False)
    self.last_update = nn.Parameter(torch.zeros(self.n_nodes).to(self.device),
                                    requires_grad=False)
